...
class GrobDatabase {
  ...
  public constructor(download_folder: string) {
    ...
    this.insert_request_stmt = this.db.prepare(`INSERT INTO requests (request, response_headers, response_body, response_body_filepath, expires_on) VALUES (:request, :response_headers, :response_body, :response_body_filepath, :expires_on)`)
  ...
  public select_request(request: RequestCreate): GrobResponse | undefined {
    ...
    const ret = this.select_request_stmt.get({ request: serialized_request, now }) as RequestsTR | undefined
    if (ret) {
